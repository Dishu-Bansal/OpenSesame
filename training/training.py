...
# --- 1. Define Helper Functions for Feature Extraction ---
...
def parse_history_sequence(history_str):
    ...
    # calls = ast.literal_eval(history_str)
...
# Convert the provided CSV string to a DataFrame
def predict_next_api_call(user_history_str, user_prompt_str, user_intent_str, candidates_from_llm, trained_model, embedding_model, fitted_encoder, fitted_rescaler, feature_columns_at_train_time):
    # 1. Prepare data for prediction: Create a DataFrame for candidates + context
    prediction_data = []
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("D:/Projects/OpenSesame/training/data.csv")
    ...
    pos = train_data[train_data['is_actual_next_call'] == 1]
    neg = train_data[train_data['is_actual_next_call'] == 0]

    # Downsample negative class to match positive count
    neg_downsampled = resample(neg, replace=False, n_samples=len(pos), random_state=42)
    balanced_calib_set = pd.concat([pos, neg_downsampled])
    X_calib = balanced_calib_set.drop(columns=['is_actual_next_call'])
    y_calib = balanced_calib_set['is_actual_next_call']

    from sklearn.linear_model import LogisticRegression

    # Raw scores from model (not calibrated)
    ...
    # Save the rescaler (and the model and encoder) for inference
    joblib.dump(score_rescaler, 'score_rescaler.pkl')
    joblib.dump(model, 'lgbm_model.pkl') # Save your LightGBM model
    joblib.dump(global_one_hot_encoder, 'one_hot_encoder.pkl') # Save your encoder

    print("\n--- Simulating Real-time Prediction ---")

    # Ensure global_one_hot_encoder and global_feature_columns are populated from training
    if global_one_hot_encoder is None or global_feature_columns is None:
        raise RuntimeError("Encoder or feature columns not set after training.")

    # --- Example User Context (from runtime input) ---
    user_history = "GET /invoices; PUT /invoices/123/status"
    user_prompt_input = "Let's finish billing for Q2"
    user_intent_input = "User wants to bill all customers in Q2"

    # --- Example Candidates generated by LLM (from OpenAPI spec) ---
    # In a real scenario, the LLM would dynamically suggest these based on the spec and context.
    # candidate_summary would come from the OpenAPI spec's description for that endpoint.
    candidate_list_from_llm = [
        {"endpoint": "/customers/{id}/invoices", "method": "GET", "summary": "Retrieves invoices for a specific customer.", "reason":""},
        {"endpoint": "/invoices", "method": "POST", "summary": "Lists all invoices across all customers.", "reason": ""},
        {"endpoint": "/customers/{id}/subscriptions", "method": "GET", "summary": "Lists subscriptions for a specific customer.", "reason":""},
        {"endpoint": "/charges", "method": "GET", "summary": "Retrieves charges data.", "reason": ""},
        {"endpoint": "/invoices/{id}/pay", "method": "POST", "summary": "Processes payment for a specific invoice.", "reason": ""},
    ]
    # Get the list of columns the model was trained on, crucial for inference
    trained_feature_columns = X_train.columns.tolist()
    joblib.dump(trained_feature_columns, 'calls_list.joblib')
    ...
    ranked_results = predict_next_api_call(
        ...
        model, 
        embedding_model,
        global_one_hot_encoder, # Pass the fitted encoder
        score_rescaler,
        trained_feature_columns # Pass the full list of columns,
    )

    print("\nRanked Next API Call Predictions:")
    for result in ranked_results:
        print(f"  {result['candidate_endpoint']}: Score = {result['score']:.4f}")
